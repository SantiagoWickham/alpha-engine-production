...
$PY   = Join-Path $REC "scripts\run_dr1f_market_refresh.py"
$RUN  = Join-Path $REC "RUN_ALPHA_ENGINE_DR1F_MARKET_REFRESH.ps1"

Write-Host "============================================================"
Write-Host "ALPHA ENGINE"
Write-Host "DATA RECOVERY V1 - DR1-F MARKET PREP"
Write-Host "INSTALL LIVE MARKET REFRESHER"
Write-Host "NO SHEET MUTATION IN THIS PREP SCRIPT"
Write-Host "============================================================"

if (-not (Test-Path $REC)) { throw "No existe Recovery V1." }
if (-not (Test-Path $V13)) { throw "No existe V13." }
...
Write-Host "============================================================"
Write-Host "ALPHA ENGINE - DR1-F MARKET REFRESH"
Write-Host "NEW DATA ENGINE ONLY"
Write-Host "FULL-COVERAGE FAIL-CLOSED"
Write-Host "FUNDAMENTALS / V13 / WEB UNCHANGED"
Write-Host "============================================================"
...
try {
    ...
    Set-Location $REC
    python scripts\run_dr1f_market_refresh.py --market-workers 6 --commit

    if ($LASTEXITCODE -ne 0) {
        throw "DR1-F market refresh falló."
    }
}
finally {
...
if (($before | Out-String) -ne ($after | Out-String)) {
    throw "GUARDRAIL: cambió V13 durante DR1-F PREP."
}

Write-Host ""
Write-Host "[OK] scripts\run_dr1f_market_refresh.py"
Write-Host "[OK] RUN_ALPHA_ENGINE_DR1F_MARKET_REFRESH.ps1"
Write-Host "[OK] V13 unchanged"
Write-Host ""
Write-Host "DR1-F PREP COMPLETE"
Write-Host "No Google Sheet cells were changed."
Write-Host ""
Write-Host "NEXT:"
Write-Host "Run:"
Write-Host "  & `"$RUN`""
