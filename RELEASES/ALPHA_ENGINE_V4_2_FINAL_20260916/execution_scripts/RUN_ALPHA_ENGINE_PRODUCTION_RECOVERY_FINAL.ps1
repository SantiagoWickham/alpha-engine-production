...
$CONTROLLER = Join-Path $REC "scripts\run_production_recovery_final.py"

Write-Host ""
Write-Host "================================================================"
Write-Host "ALPHA ENGINE - PRODUCTION RECOVERY FINAL"
Write-Host "ONE FILE / NO SHEETS / NO V13 MUTATION"
Write-Host "================================================================"
...
# Explicitly disable the old OpenAI runtime path for this recovery.
base.OPENAI_API_KEY = ""
...
if __name__ == "__main__":
    raise SystemExit(main())
'@ | Set-Content -Path $CONTROLLER -Encoding UTF8


Write-Host "[1/2] Python syntax validation"
python -m py_compile $BUILDER
if ($LASTEXITCODE -ne 0) { throw "Builder syntax failed." }
python -m py_compile $SERVER
if ($LASTEXITCODE -ne 0) { throw "Server syntax failed." }
python -m py_compile $CONTROLLER
if ($LASTEXITCODE -ne 0) { throw "Controller syntax failed." }
Write-Host "  PASS"

Write-Host ""
Write-Host "[2/2] Production recovery"
python $CONTROLLER
if ($LASTEXITCODE -ne 0) { throw "Production recovery failed. See the last gate above." }
