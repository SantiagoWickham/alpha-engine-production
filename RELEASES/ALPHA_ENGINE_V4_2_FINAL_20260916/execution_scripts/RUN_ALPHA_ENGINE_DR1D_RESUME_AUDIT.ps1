...
$PY  = Join-Path $REC "scripts\resume_dr1d_audit.py"

Write-Host "============================================================"
Write-Host "ALPHA ENGINE - DR1-D AUDIT RESUME"
Write-Host "CORE DATA WILL NOT BE REWRITTEN"
Write-Host "STATE-SAFE AUDIT RESUME / NO BLIND APPEND RETRIES"
Write-Host "============================================================"
...
if (-not $url.EndsWith("/exec")) { throw "La URL debe terminar en /exec." }
...
try {
    ...
    Set-Location $REC
    python scripts\resume_dr1d_audit.py
    if ($LASTEXITCODE -ne 0) { throw "DR1-D audit resume falló." }
}
finally {
